...
// 数据目录优先使用环境变量 DATA_DIR（Railway 挂载 Volume 时注入），本地回退到项目 data/
const DATA_DIR = process.env.DATA_DIR ? join(process.env.DATA_DIR) : join(ROOT, 'data');
...
function body(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', chunk => {
      size += chunk.length;
      if (size > 1024 * 1024) { tooLarge = true; raw = ''; return; }
      raw += chunk;
    });
    req.on('end', () => {
      if (tooLarge) return reject(new Error('请求体过大'));
