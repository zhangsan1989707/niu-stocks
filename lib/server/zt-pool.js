...
async function getZTPool({ date, force = false } = {}) {
  ...
  if (!force) {
    ...
    try {
      ...
      if (disk && disk.pool && disk.pool.length) return { ...disk, cached: true };
    } catch {}
  }

  // 1. 当日涨停池 + 昨日池（拿昨日连板数）
  const [poolRows, prevRows] = await Promise.allSettled([
    fetchPool(target, 'fbt:asc'),
    fetchPool(yesterdayDate(), 'zbc:asc'),
  ]);
  if (poolRows.status !== 'fulfilled' || !poolRows.value.length) {
    return { ok: false, error: '当日涨停池数据暂不可用（可能非交易日或接口异常）', date: target };
  }
  const prevMap = {};
  if (prevRows.status === 'fulfilled') {
    prevRows.value.forEach(r => { prevMap[String(r.c)] = r.lbc; });
  }
  ...
  const result = {
    ok: true, date: target, updatedAt: new Date().toISOString(),
    total: pool.length,
