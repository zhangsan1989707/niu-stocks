...
async function runBacktest(code, days) {
  ...
  let signalCount = 0, wins = 0, equity = 1, maxDrawdown = 0;
  let peak = -Infinity, entryPrice = null, entryDate = null;
  const signalLog = [];
  ...
  return {
    ...
    signalCount,
    wins,
    winRate: signalCount > 0 ? Math.round(wins / signalCount * 1000) / 10 : 0,
    totalReturn: Math.round((equity - 1) * 10000) / 100,
    benchmarkReturn: slice.length > 31 ? Math.round((((slice.at(-1).close * (1 - feeRate - slippageRate)) / (slice[31].open * (1 + feeRate + slippageRate))) - 1) * 10000) / 100 : 0,
    feeRate: feeRate * 100,
