...
async function runBacktest(code, days, strategy = "macd") {
  ...
  const STRATEGIES = {
    // MACD 金叉死叉（默认）
    macd: {
      buy(candles, i, closes, cfg, fast, slow, macd, signal) {
        ...
        return isGolden && prevGolden && closes[i] >= ma60;
      },
      sell(candles, i, closes, cfg, fast, slow, macd, signal) {
        const isGolden = macd[i] >= signal[i];
        const prevGolden = macd[i - 1] < signal[i - 1];
        return !isGolden && !prevGolden;
      },
    },
    // 回春法：MA60 上方 + MACD 金叉 + 放量（量 >= 20日均量 x 1.5）
    huichun: {
      buy(candles, i, closes, cfg, fast, slow, macd, signal) {
        const isGolden = macd[i] >= signal[i];
        const prevGolden = macd[i - 1] < signal[i - 1];
        const ma60 = average(closes, Math.min(cfg.ma60Period, closes.length));
        const volBase = average(candles.slice(0, i).map(c => c.volume), 20);
        const volUp = volBase > 0 && candles[i].volume >= volBase * 1.5;
        return isGolden && prevGolden && closes[i] >= ma60 && volUp;
      },
      sell(candles, i, closes, cfg, fast, slow, macd, signal) {
        const isGolden = macd[i] >= signal[i];
        const prevGolden = macd[i - 1] < signal[i - 1];
    ...
    // 20日突破（海龟简化）：收盘创 20 日新高且收阳
    breakout: {
      buy(candles, i, closes, cfg, fast, slow, macd, signal) {
        if (candles[i].close <= candles[i].open) return false;
        const prevHigh20 = Math.max(...candles.slice(Math.max(0, i - 20), i).map(c => c.high));
        return candles[i].close > prevHigh20;
      },
      sell(candles, i, closes, cfg, fast, slow, macd, signal) {
        const prevLow20 = Math.min(...candles.slice(Math.max(0, i - 20), i).map(c => c.low));
        return candles[i].close < prevLow20;
      },
    },
  };
  const strat = STRATEGIES[strategy] || STRATEGIES.macd;
  ...
  for (let i = 30; i < slice.length - 1; i++) {
    ...
    // 买入信号
    if (!entryPrice && strat.buy(slice.slice(0, i + 1), i, closes, cfg, fast, slow, macd, signal)) {
      signalCount++;
      entryPrice = nextDay.open * (1 + feeRate + slippageRate);
      if (firstEntryPrice == null) firstEntryPrice = entryPrice;
      peak = entryPrice;
      signalLog.push({ date: nextDay.date, action: 'buy', price: entryPrice, strategy });
    }

    // 卖出信号
    if (entryPrice && strat.sell(slice.slice(0, i + 1), i, closes, cfg, fast, slow, macd, signal)) {
      const exitPrice = nextDay.open * (1 - feeRate - slippageRate);
      const ret = (exitPrice - entryPrice) / entryPrice;
      equity *= 1 + ret;
      closedTrades++;
      if (ret > 0) wins++;
      signalLog.push({ date: nextDay.date, action: 'sell', price: exitPrice, profit: ret * 100, strategy });
      entryPrice = null;
    }
  ...
  if (entryPrice) {
    ...
    closedTrades++;
    if (ret > 0) wins++;
    signalLog.push({ date: slice[slice.length - 1].date, action: 'sell', price: lastClose, profit: ret * 100, strategy });
    const cur = lastClose;
    if (cur > peak) peak = cur;
    const dd = (cur - peak) / peak;
    if (dd < maxDrawdown) maxDrawdown = dd;
  ...
  return {
    ok: true, code, days: slice.length, strategy,
    signals: signalLog,
    signalCount,
    closedTrades,
    wins,
    ...
    strategyNote: strategy === 'macd' ? 'MACD 金叉死叉策略' : strategy === 'huichun' ? '回春法：MA60上方+MACD金叉+放量1.5倍' : '20日突破策略（海龟简化）',
  };
}
