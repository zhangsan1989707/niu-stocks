...
async function runBacktest(code, days) {
  ...
  for (let i = 30; i < slice.length; i++) {
    ...
    if (isGolden && prevGolden && candles[candles.length - 1].close >= ma60) {
      ...
      signalLog.push({ date: candles[candles.length - 1].date, action: 'buy', price: entryPrice });
    }
    if (!isGolden && !prevGolden && entryPrice) {
      ...
      totalReturn += ret;
      if (ret > 0) wins++;
      signalLog.push({ date: candles[candles.length - 1].date, action: 'sell', price: exitPrice, profit: ret * 100 });
      entryPrice = null;
    }
  ...
  return {
    ...
    disclaimer: '简化回测存在幸存者偏差和前视偏差，仅供参考',
  };
}
