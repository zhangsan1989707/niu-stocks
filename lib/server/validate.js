...
async function validateHealthScore(forwardDays = 5) {
  ...
  if (files.length < 2) {
    return { ok: true, message: '历史体检记录不足，需要至少 2 天的数据', samples: 0 };
  }

  // 不取最后 forwardDays 天的记录（因为没有足够的后续数据）
  const usableFiles = files.slice(0, -forwardDays);
  if (usableFiles.length === 0) {
    return { ok: true, message: `需要至少 ${forwardDays + 1} 天的历史数据`, samples: 0 };
  }

  const samples = [];
  // 只取最近 30 天的可评估记录，避免计算量过大
  const recentFiles = usableFiles.slice(-30);
  ...
  if (samples.length === 0) {
    return { ok: true, message: '无法获取足够的数据进行验证', samples: 0 };
  }
