...
const { getZTPool, listZTHistory } = require('./zt-pool');
...
async function handleApi(req, res, url, start) {
  ...
  // --- 涨停股池 ---
  if (method === 'GET' && url.pathname === '/api/ztpool') {
    const date = url.searchParams.get('date') || '';
    const force = url.searchParams.get('force') === '1';
    try {
      const result = await getZTPool({ date: /^\d{8}$/.test(date) ? date : undefined, force });
      log('GET', url.pathname, 200, Date.now() - start);
      return json(res, 200, result);
    } catch (e) { log('GET', url.pathname, 502, Date.now() - start); return json(res, 502, { error: '涨停池获取失败：' + e.message }); }
  }
  if (method === 'GET' && url.pathname === '/api/ztpool/history') {
    const list = await listZTHistory();
    log('GET', url.pathname, 200, Date.now() - start);
    return json(res, 200, { ok: true, history: list });
  }
