...
const { loadDb, saveDb, loadPortfolio, savePortfolio, loadAlerts, saveAlerts, loadNotes, saveNotes, loadConfig, saveConfig, readJson, writeJson, FAV_HISTORY_DIR, CHECK_HISTORY_DIR } = require('./store');
...
async function handleApi(req, res, url, start) {
  ...
  if (method === 'DELETE' && /^\/api\/favorites\/\d{6}$/.test(url.pathname)) {
    const db = await loadDb();
    db.favorites = db.favorites.filter(x => x.code !== url.pathname.split('/').at(-1));
    await saveDb(db);
    log('DELETE', url.pathname, 200, Date.now() - start);
  ...
  if (method === 'PUT' && /^\/api\/portfolio\/[^/]+$/.test(url.pathname)) {
    ...
    if (costPrice != null) { const c = Number(costPrice); if (!Number.isFinite(c) || c <= 0) return json(res, 400, { error: '成本价需为正数' }); pos.costPrice = c; }
