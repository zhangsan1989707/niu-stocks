 ...
 * 基于局部极值点（zigzag）判断
 * P1-4: zigzag 阈值自适应 — 根据波动率自动调整
 */

/**
 * 计算自适应 zigzag 阈值
 * 基于近 20 日 ATR（平均真实波幅）占均价比比例
 * - 低波动（<2%）：阈值 3%，避免噪声误判
 * - 中波动（2%~5%）：阈值 4%~5%
 * - 高波动（>5%）：阈值 6%~8%，避免过多假极值
 * @param {Array<{high,low,close}>} candles
 * @returns {number} threshold (0.03 ~ 0.08)
 */
function adaptiveThreshold(candles) {
  if (candles.length < 20) return 0.05;
  ...
  const volatility = avgPrice > 0 ? atr / avgPrice : 0.04;
  // 线性映射：波动率 1% → 阈值 3%, 波动率 8% → 阈值 8%
  const threshold = Math.max(0.03, Math.min(0.08, 0.025 + volatility));
  return threshold;
}
 ...
 * @param {number} threshold - 最小波动幅度（默认自适应）
 * @returns {Array<{index, price, type: 'peak'|'valley'}>}
 */
function zigzag(candles, threshold) {
  if (threshold === undefined || threshold === null) {
    threshold = adaptiveThreshold(candles);
  }
...
function detectClassicPatterns(candles) {
  ...
  // P1-4: 自适应阈值，替代硬编码 0.05
  const threshold = adaptiveThreshold(candles);
...
module.exports = { detectClassicPatterns, zigzag, adaptiveThreshold };
